...
define DROPBEAR_INSTALL_TARGET_CMDS
	...
	if [ ! -f $(TARGET_DIR)/etc/init.d/S55qtinit ]; then \
		$(INSTALL) -m 0755 -D package/dropbear/S55qtinit $(TARGET_DIR)/etc/init.d/S55qtinit; \
	fi
	if [ ! -f $(TARGET_DIR)/etc/pointercal ]; then \
		$(INSTALL) -m 0755 -D package/dropbear/pointercal $(TARGET_DIR)/etc/pointercal; \
	fi
	if [ ! -f $(TARGET_DIR)/root/mlan.tar ]; then \
		$(INSTALL) -m 0755 -D package/dropbear/mlan.tar $(TARGET_DIR)/root/mlan.tar; \
	fi
endef
...
define DROPBEAR_UNINSTALL_TARGET_CMDS
	...
	rm -f $(TARGET_DIR)/etc/init.d/S55qtinit
