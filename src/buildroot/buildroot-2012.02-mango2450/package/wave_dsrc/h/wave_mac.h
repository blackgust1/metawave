...
#define	SR5500_TEST_FRAME_ID			0xA8
...
#define		WAVE_MAC_RX_MANAGE_DEBUG_MODE						0x10
...
#define 		WAVE_MAC_RX_QUEUE_NUM									1024
...
#define 	RF_FREQ_5840		40
#define 	RF_FREQ_5850		50
#define 	RF_FREQ_5855		55
#define 	RF_FREQ_5860		60
#define 	RF_FREQ_5865		65
#define 	RF_FREQ_5870		70
#define 	RF_FREQ_5875		75
#define 	RF_FREQ_5880		80
#define 	RF_FREQ_5885		85
#define 	RF_FREQ_5890		90

/* WAVE MAC Header */
#define	WAVE_MAC_RETRY_BIT			0x08
#define WAVE_MAC_SEQ_CTRL_OFFSET		22
...
extern timer_t wave_tx_timer_id;

extern int wave_mac_rx_retry_count;
extern int wave_mac_rx_duplication_count;
extern int wave_mac_rx_no_retry_duplication_count;

extern int tx_queue_pause_threshold;



extern int Send_MPDU(int frame_kind, int ch_kind, int access, int len, int tx_power, int modulation, U1 from_to_ds, U1 *send_buf);
extern int Send_MPDU_B(int frame_kind, int ch_kind, int access, int len, int tx_power, int modulation, U1 from_to_ds, U1 *send_buf);
extern int Store_Tx_Queue(int modem_id, int frame_kind, int ch_kind, int access, int len, int tx_power, int modulation, U1 from_to_ds, U1 *send_buf);
...
extern int add_mac_addr_to_wave_rx_dest_mac_table(U1 *mac_addr, U2 fragment, U2 seq_num);
extern int find_wave_rx_dest_mac_table_with_mac_addr(U1 *mac_addr);
extern int make_send_data_for_test (int length, int mac_header_len, U1 *send_buf, int modulation, int total_count, int current_count);
extern int Send_WAVE_Data_for_Test(U1 *send_buf, int modulation, int ap_len, int tx_power, int mac_header_len, int count);
extern int clear_wave_counter(void);
extern int update_sr5500_test_data(U2 ap_len, U1 modulation, U2 total_count, int flag);
extern int clear_sr5500_test_data(void);
extern int print_sr5500_test_result(void);
extern void wave_tx_timer_handler(int signum);
extern void sr5500_rx_test_timer_handler(int signum);
extern int proc_wsmp_data(U1 *rx_wsmp_buf);



#if 0	//SHKO, Origin
extern int start_timer(int timer_id);
extern int stop_timer(int timer_id);
#else
extern int start_timer( timer_t *timerID, void (*handler)(), int sec, int msec );
extern int stop_timer( timer_t *timerID );
#endif


extern void Multi_Ch_OnOff(U1 flag);
extern void set_multi_ch_cch(U4 r0);
extern void set_multi_ch_sch(U4 r0);
